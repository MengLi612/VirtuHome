...
// اگر خارج از آگاهی کامل بود
                ...
                {
                    if (tempDis > radius * radius || tempDis < minRadius * minRadius) return;

                    // زاویه X: افقی (روی صفحه عمود بر up)
                    Vector3 flatVec = Vector3.ProjectOnPlane(_tVec, up).normalized;
                    float dotX = Vector3.Dot(flatVec, forward); // cos(angleX)
                    if (dotX < Mathf.Cos(angleX * 0.5f * Mathf.Deg2Rad)) return;

                    // زاویه Y: عمودی (بین بردار هدف و صفحه افقی)
                    float dotY = Vector3.Dot(_tVec.normalized, flatVec); // cos(angleY)
                    if (dotY < Mathf.Cos(angleY * 0.5f * Mathf.Deg2Rad)) return;
                }
