...
namespace RaycastPro.Editor
{
    ...
    public sealed class RCProPanel : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            GUILayout.Label($"<b>RAYCAST_PRO 1.1.4A</b> developed by <color=#2BC6D2>KIYNL</color>", labelStyle);
            RCProEditor.GUILine(lineColor);
            ...
            if (GUILayout.Button($"{heart} Thanks for Submit a Review {heart}", buttonStyle))
                Application.OpenURL("https://assetstore.unity.com/packages/tools/physics/raycastpro-214714#reviews");
            if (GUILayout.Button($"Follow tutorials on  {play} YouTube", buttonStyle))
        ...
        {
            var scriptType = type.GetClass();

            string prop = "no Information";
            if (scriptType != null && scriptType.IsSubclassOf(typeof(RaycastCore)))
            {
                prop = RaycastCore.GetInfo(scriptType);
            }

            var content = new GUIContent(GetIconFromScript(type), prop);
